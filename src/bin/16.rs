...
fn find_nodes_in_path(
    ...
    */
    ...
    next_directions.iter().for_each(|next_dir| {
        if let Some(next_location) = get_next_location(grid, current, *next_dir) {
            find_nodes_in_path(grid, (next_location.0, next_location.1), *next_dir, visited);
        }
    });
}
